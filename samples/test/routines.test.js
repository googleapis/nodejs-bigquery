...
describe('list, get, update routines', async() => {
  ...
  it('should get a routine', async () => {
    const output = execSync(`node routines/getRoutine.js ${datasetId} ${routineId}`);
  ...
  it('should update routine', async () => {
    ...
    const output = execSync(`node routines/updateRoutine.js ${datasetId} ${routineId}`);
  ...
  describe('Delete Routine', () => {
    ...
    try {
      const resp = await bigquery.getRoutine(getRequest)
          } catch (err) {
            assert.strictEqual(
              err.details,
